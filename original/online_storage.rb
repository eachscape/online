...
class OnlineStorage
  ...
  def initialize(store_type)
    ...
    if store_type == :queue
      pieces << 'queue'
      case Rails.env
        when 'live' # do nothing!
        when 'development' # development writes to staging queues... we don't have a development generator
          pieces << 'staging'
        else
          pieces << Rails.env
      end
  ...
  def self.default
    OnlineStorage.retryable do 
      @@s3_instance ||= Rails.env.test? ? MockOnlineStorage.new(:s3) : OnlineStorage.new(:s3)
    end
    @@s3_instance
  end
  ...
  def self.cdn
    OnlineStorage.retryable do 
      @@cdn_instance ||= Rails.env.test? ? MockOnlineStorage.new(:s3_cdn) : OnlineStorage.new(:s3_cdn)
    end
    @@cdn_instance
  end
  ...
  def self.queue
    OnlineStorage.retryable do 
      ...
      @@queue_instance ||= OnlineStorage.new(:queue)
    end
    @@queue_instance
  end
  ...
  def url_for(key, options = {})
    ...
    OnlineStorage.retryable {S3Object.url_for(key, @bucket_name, options)}
  end

  def write(key, object_or_stream, options = {})
    mime_type = options.delete(:mime_type) || 'text/plain'
    options.merge!(:content_type => mime_type)
    response = OnlineStorage.retryable {S3Object.store(key, object_or_stream, @bucket_name, options)}
  end
  ...
  def exist?(key)
    begin
      response = OnlineStorage.retryable {S3Object.find(key, @bucket_name)}
      return true
  ...
  def find(key)
    OnlineStorage.retryable {S3Object.find(key, @bucket_name)}
  end
